...
# device = "cpu"
save_dir = "E:\junior_1\冯如杯\测试0514\静息眼动0514.pkl"  # 模型保存位置
filename_csv = []  # 文件名列表
data_len = 128  # 数据采样长度
all_data = []   # 训练数据
labels = []     # 对应训练数据的类别
timeset = 'EEG.Counter'
'''设置which传感器数据是需要的'''
choose_sensor = [1, 2, 3, 4, 11, 12, 13, 14]
...
def overlap(data_set, index, label):
    length = len(data_set)
    for i in range(index, length - 1):
        data1 = data_set[i]
        data2 = data_set[i + 1]
        new_data = np.concatenate((data1[:, 44:], data2[:, :44]), axis=1)
...
'''读取数据'''
def read_csv(address, label, index):     # address代表读取地址，num代表当前的归类
    ...
    for root, dirs, files in os.walk(file_chdir):  # os.walk会便利该目录下的所有文件
        for file in files:
            if os.path.splitext(file)[-1] == '.csv':  # 找csv文件
                ...
                file0 = (pd.read_csv(file,header=1))  # 打开当前文件
                ...
                while raw < data_lenth:
                    if raw % 128 == 0:
                        ...
                        raw += 1
    overlap(all_data, index=index, label=label)
...
class CNNetwork(nn.Module):
    def __init__(self):
        super().__init__()
        self.module = nn.ModuleDict({
            'conv1': nn.Conv2d(in_channels=14, out_channels=20, kernel_size=(1, 3)),
            'relu1': nn.ReLU(),
        ...
        self.linear1 = nn.Linear(680, 100)
...
if __name__ == '__main__':
    read_csv('E:\junior_1\冯如杯\测试0514\静息', 0, index = 0)
    read_csv('E:\junior_1\冯如杯\测试0514\眼动', 1, index = len(all_data))


    all_data = data_normal(all_data)
    ...
    x_val = torch.tensor(x_data[-int(0.3*lenth):], dtype=torch.float)
    y_val = torch.tensor(y_label[-int(0.3*lenth):], dtype=torch.int64)
    ...
    NUM_CLASS = 2
    #创建对象
    model = ConvNets_FFT(num_class=NUM_CLASS).to(device)
    ...
    scheduler = torch.optim.lr_scheduler.MultiStepLR(optimizer, milestones=[100, 200, 400, 700], gamma=0.5)
    #初始化列表储存准确度和损失
    accuracy = []
    loss_re = []
    ...
    for epoch in range(EPOCHS):
        ...
        if epoch % 10 == 0:
            ...
            if ttt == 0:
                ...
                acc = correct / ttt * 100
            print('epoch = {:d}, loss = {: .6f}, val_acc = {: .3f}'.format(epoch, float(loss), acc))
