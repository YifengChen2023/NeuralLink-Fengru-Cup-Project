...
if __name__ == '__main__':
    ...
    id2action = {0: '空', 1: '向前', 2: '向左', 3: '向右', 4: '向后', 5: '舌动', 6: '眼动'}
    ...
    while True:
        ...
        if total_ist % 32 == 16:
            ...
            data1.append(total_ist % 1280 + 1)  # 传递现在的时间戳
            aa = data0 + data1  # 加入指令
            # aa = 1-14数据1 + ins + 1-14数据2 + ins + time_stamp
            s = str(aa)
            buf = connection.recv(40960)
            info = buf.decode('utf-8')
            connection.send(bytes(s, encoding="utf-8"))
            if 'switch' in info:    # 开关状态
                # print(info)
                onoff = re.findall(r"switch=(.+?)ok", info)
                print('onoff', onoff)
            elif 'mode' in info:    # 模式选择，这个代码没有用到
                mode = re.findall(r"mode=(.+?)ok", info)
                print('mode=', mode)


            '''
            下面是为了保证指令已经穿过去后在进行动作，flag为了保证一个指令只执行一次
            如果莫名其妙错误，可以用之前的代码。加入第166行data1.append(total_ist % 1280 + 1)即可
            '''
            if flag:
                if onoff != ['0'] and onoff != 0:  # onoff不是0才给无人机传指令
                    print(onoff)
                    if final_ins == 1:
                        if mode == ['1']:
                            tello.flip_forward()
                        elif mode == ['2']:
                            tello.move_forward(DIS)
                    elif final_ins == 2:
                        if mode == ['1']:
                            tello.flip_left()
                        elif mode == ['2']:
